...
const ALL_TAGS = ['all', 'backend', 'nestjs', 'agentic', 'tool-use', 'typescript', 'frontend', 'react', 'nextjs', 'debugging', 'race-condition'];
...
export default function PackCard({ packs }: PackCardProps) {
  ...
  const filtered = useMemo(() => {
    return packs.filter(pack => {
      ...
      const diffMatch = activeDifficulty === 'all' ||
    ...
    <div className="packs-layout">
      ...
      <aside className="packs-sidebar">
        ...
        <div className="packs-filter-count">
          <span className="mono-data">{filtered.length}</span> pack{filtered.length !== 1 ? 's' : ''} found
        </div>
      </aside>
      ...
      <section className="packs-content">
        {filtered.length === 0 ? (
          ...
          filtered.map(pack => {
            ...
            return (
              <div key={pack.name} className={`pack-card ${isExpanded ? 'pack-card-expanded' : ''}`}>
                <div
                ...
                >
                  <div className="pack-card-main">
                    <div className="pack-card-title-row">
                      ...
                      <span className="pack-card-version chip chip-primary">v{pack.version}</span>
                    </div>
                    <p className="pack-card-desc">{pack.description}</p>
                    <div className="pack-card-stats">
                      <span className="pack-stat">
                        <span className="material-symbols-outlined pack-stat-icon">description</span>
                        <span className="pack-stat-value">{pack.total_prompts} prompts</span>
                      </span>
                      <span className="pack-stat">
                        <span className="material-symbols-outlined pack-stat-icon">construction</span>
                        <span className="pack-stat-value">{totalSkills} skills</span>
                      </span>
                      <span className="pack-stat">
                        <span className="material-symbols-outlined pack-stat-icon">category</span>
                        <span className="pack-stat-value">{pack.categories.length} categories</span>
                      </span>
                    </div>
                  ...
                  <span className={`material-symbols-outlined pack-expand-icon ${isExpanded ? 'pack-expand-open' : ''}`}>
                    expand_more
                  </span>
                </div>
                  ...
                  <div className="pack-card-body animate-entrance">
                        ...
                        {pack.categories.map(cat => (
                          <div key={cat.id} className="pack-category-item">
                            <div className="pack-cat-header">
                              <span className="pack-cat-label">{cat.label}</span>
                              <span className="pack-cat-count mono-data">{cat.count}</span>
                            </div>
                            <div className="pack-cat-diffs">
                              {cat.difficulty.map(d => (
                                <span key={d} className={`pack-diff-badge pack-diff-${d}`}>{d}</span>
                              ))}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="pack-skills-list">
                        {pack.skills.map(skill => (
                          <div key={skill.name} className="pack-skill-item">
                            <code className="pack-skill-name">{skill.name}</code>
                            {skill.version && <span className="pack-skill-version chip chip-primary">v{skill.version}</span>}
                            <span className="pack-skill-desc">{skill.description}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {(pack.author || pack.license) && (
                      <div className="pack-meta-row">
                        {pack.author && <span className="pack-meta-item">👤 {pack.author}</span>}
                        {pack.license && <span className="pack-meta-item">📄 {pack.license}</span>}
                      </div>
                    )}
                  </div>
                )}
