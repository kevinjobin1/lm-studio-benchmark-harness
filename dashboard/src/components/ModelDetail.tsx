...
import { loadResults, groupByModel, buildLeaderboard } from '../lib/loadResults';
...
export default function ModelDetail({ modelId }: ModelDetailProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setModelRuns(runs);
        setBestRun(
          runs.reduce((a, b) => (a.metrics.overall_score > b.metrics.overall_score ? a : b))
        );
      } catch {
        // fallback — redirect
      } finally {
  ...
  const totalFailures = Object.values(bestRun.failures).reduce((a, b) => a + b, 0);

  const scoreCategories = [
    { label: 'Coding', value: bestRun.metrics.coding_score, emoji: '💻' },
    { label: 'Reasoning', value: bestRun.metrics.reasoning_score, emoji: '🧠' },
    { label: 'Instructions', value: bestRun.metrics.instruction_score, emoji: '📋' },
  ...
  function barColor(v: number) {
    return v >= 0.8 ? 'var(--success)' : v >= 0.6 ? 'var(--brand-primary)' : v >= 0.4 ? 'var(--warning)' : 'var(--error)';
  }
    ...
    <>
      ...
      <section className="page-hero" style={{ paddingTop: 16 }}>
        ...
        <p>
          ...
          {bestRun.model_metadata.quantization && `${bestRun.model_metadata.quantization} · `}
        ...
        <div className="grid-3">
          {scoreCategories.map(s => (
            <div key={s.label} className="metric-card card">
              ...
              <span className="metric-value-big">{(s.value * 100).toFixed(1)}%</span>
              <div className="metric-bar-track" style={{ marginTop: 8 }}>
                <div
                  className="metric-bar-fill"
                  style={{ width: `${s.value * 100}%`, background: barColor(s.value) }}
                />
              </div>
            </div>
          ))}
      ...
      <section style={{ marginBottom: 48 }}>
        <div className="section-header">
          <h2>Performance</h2>
          <p>Latency and throughput on {bestRun.hardware.processor} ({bestRun.hardware.memory_gb}GB)</p>
        </div>
          ...
          <div className="card">
            <div className="perf-group">
              <div className="perf-stat">
                ...
                <span className="perf-stat-value">{bestRun.performance.tokens_per_sec.toFixed(0)}</span>
              </div>
              <div className="perf-stat">
                <span className="perf-stat-label">Normalized t/s</span>
                <span className="perf-stat-value">{bestRun.performance.normalized_tps.toFixed(0)}</span>
              </div>
            </div>
          </div>
          <div className="card">
            <div className="perf-group">
              <div className="perf-stat">
                ...
                <span className="perf-stat-value">{bestRun.performance.ttft_ms.toFixed(0)}<span style={{ fontSize: '0.5em' }}>ms</span></span>
              </div>
              <div className="perf-stat">
                <span className="perf-stat-label">Total Latency</span>
                <span className="perf-stat-value">{bestRun.performance.total_latency_ms.toFixed(0)}<span style={{ fontSize: '0.5em' }}>ms</span></span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ marginBottom: 48 }}>
        <div className="section-header">
          ...
          <p>Across {bestRun.stats.runs} runs{bestRun.seed ? ` with seed=${bestRun.seed}` : ''}</p>
        </div>
        <div className="card">
          <div className="stats-grid">
            <div className="stat-item">
              <span className="stat-label">Mean</span>
              <span className="stat-value">{bestRun.stats.mean.toFixed(3)}</span>
            </div>
            <div className="stat-item">
              <span className="stat-label">Std Dev</span>
              <span className="stat-value">±{bestRun.stats.std.toFixed(3)}</span>
            </div>
            <div className="stat-item">
            ...
            <div className="stat-item">
              ...
              <span className="stat-value">{bestRun.stats.median.toFixed(3)}</span>
            </div>
            <div className="stat-item">
              <span className="stat-label">CoV</span>
              <span className="stat-value">{bestRun.stats.coefficient_of_variation.toFixed(3)}</span>
            </div>
          </div>
          {bestRun.stats.confidence_95 && (
            <div className="confidence-interval">
              <span>95% CI: [{bestRun.stats.confidence_95[0].toFixed(3)}, {bestRun.stats.confidence_95[1].toFixed(3)}]</span>
            </div>
          )}
        </div>
      </section>
