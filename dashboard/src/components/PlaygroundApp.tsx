...
const AVAILABLE_SKILLS: SkillInfo[] = [
  ...
  },
  {
    ...
    description: 'Prisma ORM database operations: findUnique, create, update, delete, transactions',
    inputSchema: { operation: 'string', model: 'string', data: 'object' },
    selected: false,
  },
  {
    name: 'nestjs_jwt_guard',
    description: 'JWT authentication guard: validate tokens, extract user, role-based access',
  ...
  },
  {
    ...
    description: 'Validation pipe: validate DTOs, transform types, sanitize input',
...
const TASK_TEMPLATES = [
  {
    label: 'Read & Fix Bug',
    template: 'Read the file src/buggy.ts, fix the race condition bug, write the fix, and show the diff of changes.',
  },
  {
    label: 'JSON Transform',
    template: 'Read config.json, parse its JSON content, transform the structure to add versioning, and write the result to config_v2.json.',
  },
  {
    label: 'Code Review Diff',
    template: 'Read both src/original.ts and src/modified.ts, compute their diff, and verify the changes are correct.',
  },
  {
    label: 'Auth Guard Debug',
    template: 'Read src/auth/jwt-auth.guard.ts, identify why token validation is broken, write the fix with proper super.canActivate() call, and show the diff.',
  },
  {
    label: 'Cache Race Fix',
    template: 'Read src/common/cache.service.ts, fix the cache stampede problem using Promise deduplication, write the fix, and show the diff.',
  },
];
...
export default function PlaygroundApp() {
  const [skills, setSkills] = useState<SkillInfo[]>(AVAILABLE_SKILLS.map(s => ({ ...s })));
  ...
  const runPlayground = useCallback(async () => {
    ...
    if (selectedSkills.includes('json_parse')) {
      actions.push({ skill: 'json_parse', input: { json_string: '{"version": 1}' } });
    }
    if (selectedSkills.includes('write_file')) {
      actions.push({ skill: 'write_file', input: { path: 'src/index.ts', content: '// fixed' } });
    }
    if (selectedSkills.includes('diff')) {
      actions.push({ skill: 'diff', input: { a: 'original', b: 'modified' } });
    }
    if (selectedSkills.includes('nestjs_jwt_guard') && prompt.includes('auth')) {
      actions.push({ skill: 'nestjs_jwt_guard', input: { token: 'eyJ...' } });
    }
    if (selectedSkills.includes('nestjs_cache_interceptor') && prompt.includes('cache')) {
      actions.push({ skill: 'nestjs_cache_interceptor', input: { key: 'user:123', ttl: 300 } });
    }

    // Compute mock scores
    const totalSkills = selectedSkills.length;
    const usedCount = new Set(actions.map(a => a.skill)).size;
    const hallucinated = actions.filter(a => !selectedSkills.includes(a.skill)).length;
    ...
    const overall = (validity * 0.25 + planning * 0.25 + correctness * 0.30 + constraints * 0.20);
    ...
    <div className="playground-app">
      ...
      <div className="task-section">
          ...
          <div className="template-grid">
            {TASK_TEMPLATES.map((t, i) => (
              ...
              >
                ...
                <span className="template-preview">{t.template.substring(0, 80)}...</span>
              </button>
            ))}
          </div>
        )}
        ...
        <textarea
          ...
          value={prompt}
          onChange={e => { setPrompt(e.target.value); setMode('custom'); }}
      ...
      {results && (
        <div className="results-section">
          ...
          <div className="results-tabs">
            ...
            <button className={`tab-btn ${tab === 'raw' ? 'active' : ''}`} onClick={() => setTab('raw')}>
              Raw JSON
            </button>
          </div>
          ...
          {tab === 'actions' && (
            <div className="actions-list">
              ...
              {results.actions.length === 0 && (
                <p className="empty-state">No actions returned. Model may have failed to produce valid JSON.</p>
              )}
            </div>
          )}
          ...
          {tab === 'scores' && results.scores && (
            <div className="scores-grid">
              <div className="score-card">
                ...
                <div className={`score-value ${results.scores.overall >= 0.8 ? 'good' : results.scores.overall >= 0.5 ? 'ok' : 'bad'}`}>
              ...
              <div className="score-card">
                ...
                <div className="score-bar">
                  <div className="bar-fill" style={{ width: `${results.scores.validity * 100}%` }} />
                </div>
                <span className="score-pct">{(results.scores.validity * 100).toFixed(0)}%</span>
              </div>
              <div className="score-card">
                <div className="score-label">Planning</div>
                <div className="score-sub">Sequence correctness</div>
                <div className="score-bar">
                  <div className="bar-fill" style={{ width: `${results.scores.planning * 100}%` }} />
                </div>
                <span className="score-pct">{(results.scores.planning * 100).toFixed(0)}%</span>
              </div>
              <div className="score-card">
                <div className="score-label">Skill Correctness</div>
                <div className="score-sub">Tool + params</div>
                <div className="score-bar">
                  <div className="bar-fill" style={{ width: `${results.scores.correctness * 100}%` }} />
                </div>
                <span className="score-pct">{(results.scores.correctness * 100).toFixed(0)}%</span>
              </div>
              <div className="score-card">
                <div className="score-label">Constraint Adherence</div>
                <div className="score-sub">No hallucinations</div>
                <div className="score-bar">
                  <div className="bar-fill" style={{ width: `${results.scores.constraints * 100}%` }} />
                </div>
                <span className="score-pct">{(results.scores.constraints * 100).toFixed(0)}%</span>
              </div>
            </div>
          )}

          {tab === 'raw' && (
            <pre className="raw-output">{results.rawResponse || 'No raw response available'}</pre>
          )}
