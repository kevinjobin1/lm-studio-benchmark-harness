  ...
  const sorted = useMemo(() => {
    ...
    switch (sortKey) {
      case 'coding':
        ...
        break;
      case 'throughput':
        arr.sort((a, b) => b.performance.tokens_per_sec - a.performance.tokens_per_sec);
        break;
      default:
  ...
  return (
    <div className="leaderboard-table-wrapper">
      ...
      <table className="leaderboard-table">
        ...
        <tbody>
          {sorted.map((m, i) => {
            ...
            const barColor = pct >= 80 ? 'var(--success)' : pct >= 60 ? 'var(--brand-primary)' : pct >= 40 ? 'var(--warning)' : 'var(--error)';

            return (
              <tr key={m.model} className={i < 3 ? 'lb-row-podium' : ''}>
                <td className="lb-td-rank">
                  <span className={`rank-medal ${medal.cls}`}>{medal.emoji}</span>
                </td>
                <td className="lb-td-model">
                  <div className="lb-model-cell">
                    <a href={`/model/${encodeURIComponent(m.model)}`} className="lb-model-link">
                      {m.model}
                    </a>
                ...
                <td className="lb-td-speed">
                  <span className="lb-speed-value">{m.performance.tokens_per_sec.toFixed(0)}</span>
      ...
      <div className="lb-footnote">
        <span className="material-symbols-outlined lb-footnote-icon">sensors</span>
        <span className="lb-footnote-text">
          Benchmarks on Apple Silicon · {models.length} models · sorted by {sortKey === 'overall' ? 'overall score' : sortKey === 'coding' ? 'coding score' : 'throughput'}
        </span>
      </div>
    </div>
  );
