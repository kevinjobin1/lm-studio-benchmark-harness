...
function generateStepsFromResult(r: BenchmarkResult): TraceStep[] {
  ...
  if (categories.length > 0) {
    categories.forEach(([cat, scores], i) => {
      const subEntries = Object.entries(scores as Record<string, number>).slice(0, 3);
      steps.push({
        id: `${r.run_id}-cat${i}`,
        type: 'tool_call',
        label: `Evaluating: ${cat}`,
        tool: `eval_${cat}`,
        input: subEntries.map(([k, v]) => `${k}: ${(v * 100).toFixed(0)}%`).join('\n'),
  ...
  const failures = Object.entries(r.failures || {}).filter(([, count]) => count > 0);
...
function buildTraceRuns(results: BenchmarkResult[]): TraceRun[] {
  ...
  return results.map(r => ({
    ...
    status: r.metrics.overall_score >= 0.6 ? 'completed' as const : 'failed' as const,
  ...
  return [
    {
      ...
      status: 'completed',
      steps: [
        { id: 's1', type: 'system', label: 'System Instruction', detail: 'You are a NestJS backend engineer. Implement with proper error handling.', timing_ms: 0, status: 'success' },
        { id: 's2', type: 'tool_call', label: 'Calling: prisma.user.findUnique', tool: 'prisma_user_service', input: '{ id: "user_123" }', timing_ms: 42, status: 'success' },
        { id: 's3', type: 'reasoning', label: 'Analyzing schema relations', detail: 'User → Posts (1:M), User → Profile (1:1). Need to include both relations.', timing_ms: 28, status: 'success' },
        { id: 's4', type: 'tool_call', label: 'Calling: prisma.user.findUnique (with includes)', tool: 'prisma_user_service', input: '{ id: "user_123", include: { posts: true, profile: true } }', timing_ms: 38, status: 'success' },
        { id: 's5', type: 'response', label: 'Generated Implementation', detail: 'Exported UserService.getUserWithRelations() with Prisma includes and error handling.', timing_ms: 156, status: 'success' },
      ],
    },
    {
      id: 'run-7712-mistral',
      ...
      status: 'completed',
      steps: [
        { id: 's1', type: 'system', label: 'System Instruction', detail: 'Find and fix the race condition.', timing_ms: 0, status: 'success' },
        { id: 's2', type: 'reasoning', label: 'Identifying the race condition', detail: 'Promise.all on line 42 fires concurrent writes to same cache key.', timing_ms: 45, status: 'success' },
        { id: 's3', type: 'tool_call', label: 'Calling: grep for cache.set', tool: 'file_search', input: 'cache.set', timing_ms: 18, status: 'success' },
        { id: 's4', type: 'response', label: 'Applied Fix', detail: 'Added async-mutex. Race condition resolved.', timing_ms: 312, status: 'success' },
      ],
    },
  ];
}
...
export default function TraceTimeline({ results }: TraceTimelineProps) {
  const tracesFromData = results && results.length > 0
    ? buildTraceRuns(results)
    : null;
    ...
    <div className="traces-layout">
      ...
      <aside className="traces-sidebar">
        <div className="traces-sidebar-header">
          ...
          <span className="material-symbols-outlined traces-filter-icon">filter_list</span>
        </div>
          ...
          {traces.map(run => (
            ...
            >
              <div className="traces-run-top">
                ...
                <span className={`material-symbols-outlined traces-run-status traces-status-${run.status}`}>
                  {run.status === 'completed' ? 'check_circle' : run.status === 'running' ? 'sync' : 'cancel'}
                </span>
              </div>
          ...
          <>
            <div className="traces-timeline-header">
              <div className="traces-tl-info">
                ...
                <span className="traces-tl-prompt mono-data">{selectedRun.pack} › {selectedRun.prompt}</span>
              </div>
              <div className="traces-tl-total">
                <span className="mono-label">Total Time</span>
                <span className="traces-tl-time">{selectedRun.totalTimeMs}ms</span>
              </div>
            </div>
                  ...
                  >
                    <div className="traces-step-gutter">
                      <div
                        ...
                        style={{ background: stepColors[step.type] || 'var(--text-tertiary)' }}
                      />
                    ...
                    <div className="traces-step-content">
                      <div className="traces-step-header">
                        <span
                          ...
                          style={{ color: stepColors[step.type] || 'var(--text-tertiary)' }}
                        >
                          {stepIcons[step.type] || 'circle'}
                        </span>
                        <span className="traces-step-type-chip" style={{
                        ...
                        <span className="traces-step-time">{step.timing_ms}ms</span>
                      </div>
                      ...
                      {step.type === 'tool_call' && step.tool && isHovered && (
                        <div className="traces-tool-detail animate-entrance">
                          <div className="traces-tool-header">
                            ...
                            <code className="traces-tool-name">{step.tool}</code>
                          </div>
                          {step.input && (
                            <pre className="traces-tool-input"><code>{step.input}</code></pre>
                          )}
                        </div>
                      )}
            ...
            <div className="traces-playback">
              ...
              <div className="traces-pb-progress">
                ...
                <span className="traces-pb-time mono-data">{selectedRun.steps.length} steps</span>
              </div>
            </div>
          </>
        ) : (
          <div className="empty-state">
            <span className="material-symbols-outlined" style={{ fontSize: 48, marginBottom: 16, color: 'var(--text-tertiary)' }}>account_tree</span>
