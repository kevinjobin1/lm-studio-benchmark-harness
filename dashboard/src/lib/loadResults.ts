...
export async function loadResults(): Promise<BenchmarkResult[]> {
  try {
    ...
    console.warn('No results.json found. Using demo data.');
 ...
 */
export function groupByModel(results: BenchmarkResult[]): Map<string, BenchmarkResult[]> {
...
function generateDemoData(): BenchmarkResult[] {
  ...
  return models.map((m, i) => ({
    ...
    hardware: { platform: 'macOS 14.5', processor: 'Apple M3', memory_gb: 18, architecture: 'arm64' },
    ...
    metrics: {
      ...
      overall_score: (m.coding * 0.4 + m.reasoning * 0.3 + m.instruction * 0.2 + 0.72 * 0.1),
    },
    performance: {
