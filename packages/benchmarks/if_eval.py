...
class IFEvalBenchmark(Benchmark):
    ...
    TASKS = [
        ...
        },
        {
            ...
            "check": lambda x: (
                x.strip().startswith("Today") and x.strip().lower().endswith("tomorrow.")
            ),
        },
        {
            ...
            "check": lambda x: (
                len([s for s in x.split(".") if s.strip()]) == 3
                and bool(re.search(r"\bdogs?\b", x.lower()))
            ),
        },
        {
        ...
        },
        {
            ...
            "check": lambda x: (
                len(set(w[0].lower() for w in x.split() if w)) == 1 if x.split() else False
            ),
        },
        {
    ...
    def run(self, samples: int = 100) -> List[BenchmarkResult]:
        ...
        for i, task in enumerate(tasks):
            messages = [
                {
                    "role": "system",
                    "content": "You are a precise instruction-following assistant. Follow the user's instruction EXACTLY. Respond with ONLY what is requested — no explanations, no markdown, no extra text whatsoever.",
                },
            ...
            try:
                ...
                if self.verbose:
                    ...
                    print(
                        f"[VERBOSE if_eval #{i + 1}] Response ({len(response)} chars): {repr(response)} [{', '.join(task['constraints'])}]"
                    )
                ...
                if self.verbose:
                    print(
                        f"[VERBOSE if_eval #{i + 1}] Constraints: {'PASSED' if is_passed else 'FAILED'}"
                    )
