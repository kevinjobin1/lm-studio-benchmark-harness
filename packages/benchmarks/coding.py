...
{problem["prompt"]}"""
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a code completion assistant. Respond with ONLY the raw Python function code. Never use markdown code fences (```). Never include explanations or commentary. Just the function body code.",
                },
            ...
            try:
                ...
                if self.verbose:
                    ...
                    print(
                        f"[VERBOSE humaneval #{i + 1}] Response ({len(response)} chars): {repr(response[:200])}"
                    )
                ...
                if self.verbose:
                    print(
                        f"[VERBOSE humaneval #{i + 1}] Extracted code ({len(code)} chars): {repr(code[:120])}"
                    )
                ...
                if self.verbose:
                    print(
                        f"[VERBOSE humaneval #{i + 1}] Exec result: {'PASS' if is_correct else 'FAIL'}"
                    )
                ...
                results.append(
                    BenchmarkResult(
                        ...
                        metadata={"task_id": problem["task_id"], "code_length": len(code)},
                    )
                )
