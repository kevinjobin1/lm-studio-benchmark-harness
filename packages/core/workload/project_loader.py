...
class ProjectLoader:
    ...
    def _parse_file(
        self, file_path: Path, root: Path, language_hint: Optional[str] = None
    ...
    def _detect_project(files: List[ProjectFile]) -> tuple:
        ...
        primary_framework = (
            max(framework_scores, key=framework_scores.get) if framework_scores else None
        )
    ...
    def _parse_ts_exports(content: str) -> List[str]:
        ...
        for match in re.finditer(
            r"export\s+(?:default\s+)?(?:class|function|interface|type|const|enum|abstract\s+class)\s+(\w+)",
            content,
        ):
    ...
    def _parse_rust_exports(content: str) -> List[str]:
        ...
        exports = []
        for match in re.finditer(
            r"pub\s+(?:fn|struct|enum|trait|type|const|mod|use)\s+(\w+)", content
        ):
            exports.append(match.group(1))
        return exports
...
""",
                ...
                classes=[],
                functions=[
                    {"name": "cli", "params": "", "return_type": "None"},
                    {"name": "greet", "params": "name, greeting", "return_type": "None"},
                    {"name": "math", "params": "numbers, operation", "return_type": "None"},
                ],
                imports=[],
            ),
        ]
