...
class LMStudioClient:
    ...
    def __init__(
        ...
        logger.warning("LMStudioClient is deprecated. Use OpenAICompatibleProvider from providers.")
...
class BenchmarkSuite:
    ...
    def run_benchmark(self, name: str, samples: Optional[int] = None) -> List[BenchmarkResult]:
        ...
        sample_count = samples or self.config.get("benchmarks", {}).get(
        ...
        # Correlate provider events with this benchmark run by setting
        # _current_run_id on the provider so TokenGenerated/Completion
        # events share the same run_id as lifecycle and metric events.
        if hasattr(self.client, "_current_run_id"):
            self.client._current_run_id = run_id
        ...
        try:
            ...
            # Always clear run_id on provider after benchmark completes
            if hasattr(self.client, "_current_run_id"):
                self.client._current_run_id = ""
        ...
        # _current_run_id was already cleared in the finally block
        # above; this comment just marks the end of the correlated block.

        return results

    def run_all(
        self, samples: Optional[int] = None, benchmark_names: Optional[List[str]] = None
