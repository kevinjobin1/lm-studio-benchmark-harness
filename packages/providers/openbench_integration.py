...
class OpenBenchRunner:
    ...
    def check_openbench_installed(self):
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "openbench", "--version"], capture_output=True, text=True, timeout=10
            )
        ...
        try:
            result = subprocess.run(
                ["openbench", "--version"], capture_output=True, text=True, timeout=10
            )
            if result.returncode == 0:
                self.installed = True
                self.use_uv = False
    ...
    def install_openbench(self):
        ...
        try:
            subprocess.run(["uv", "pip", "install", "openbench"], check=True, timeout=300)
            self.installed = True
            self.use_uv = True
    ...
    def run_benchmark(
        self, benchmark: str, limit: Optional[int] = None, debug: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": "Benchmark execution timed out", "success": False}

    def run_benchmarks(
        self, benchmarks: List[str], limit: Optional[int] = None, debug: bool = False
    ) -> Dict[str, Any]:
...
def run_openbench_benchmarks(
    ...
    results = {"openbench": {}, "custom": custom_benchmarks}

    if openbench_tasks:
        try:
            eval_results = runner.run_benchmarks(benchmarks=openbench_tasks, limit=limit)
