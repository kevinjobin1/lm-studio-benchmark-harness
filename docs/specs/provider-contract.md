...
Defined in `packages/providers/base.py`. All 6 providers (LM Studio, Ollama, Open WebUI, Jan, llama.cpp, vLLM) implement this contract.
...
def health_check(self) -> bool:
    # Primary: try /v1/models
    # Fallback: try /health (llama.cpp, vLLM) or /api/tags (Ollama)
    ...
```
...
- Model metadata should include: `id`, `name`, `provider`, `parameters`, `quantization`, `size_bytes`
- Handle empty responses gracefully (some providers serve one model and return empty lists)
...
Represents a model available on a provider.
...
Low-level metrics for a single API call.
...
Hardware/performance metrics for the provider process.
...
All providers MUST use these helpers from `packages/providers/base.py` instead of string manipulation:
...
- [Event Schema](event-schema.md) — event bus contract
- [Run Schema](run-schema.md) — canonical data model
- `packages/providers/base.py` — source of truth
- `packages/providers/openai_compatible.py` — reference implementation
