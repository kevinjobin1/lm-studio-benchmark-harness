...
class JSONSchemaEvaluator(Evaluator):
    ...
    def evaluate(self, prompt: str, response: str, schema: Optional[Dict] = None, **kwargs) -> EvaluationResult:
...
class RegexConstraintEvaluator(Evaluator):
    ...
    def evaluate(self, prompt: str, response: str, constraints: Optional[Dict] = None, **kwargs) -> EvaluationResult:
...
class KeywordMatchEvaluator(Evaluator):
    ...
    def evaluate(self, prompt: str, response: str, keywords: Optional[List[str]] = None, **kwargs) -> EvaluationResult:
...
class NumericalAnswerEvaluator(Evaluator):
    ...
    def evaluate(self, prompt: str, response: str, expected: Optional[float] = None, tolerance: float = 0.01, **kwargs) -> EvaluationResult:
...
class CodeExecutionEvaluator(Evaluator):
    ...
    def evaluate(self, prompt: str, response: str, language: str = "typescript", **kwargs) -> EvaluationResult:
        ...
        if not self.has_ts_node and language == "typescript":
            return EvaluationResult(
                score=0.5,
                passed=True,
                metadata={"skipped": "ts_node_not_available"},
                failures=[]
            )
