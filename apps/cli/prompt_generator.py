...
class CodePromptGenerator:
    ...
    def generate_code_prompt(
        self, domain: Optional[str] = None, difficulty: str = "medium", language: str = "typescript"
    ) -> GeneratedPrompt:
...
class PromptGenerator:
    ...
    def substitute_variables(self, prompt: str, intensity: float = 0.5) -> str:
        ...
        func_words = re.findall(
            r"\b([a-z]+(?:Data|Request|Records|Payload|Input|Output|Response|Result|Entity|Event|Task|Work))\b",
            result,
        )
    ...
    def generate_prompt(
        self, category: PromptCategory, domain: Optional[str] = None, difficulty: str = "medium"
    ) -> GeneratedPrompt:
