...
class AppleSiliconMonitor:
    ...
    def get_thermal_pressure(self) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(
                ["system_profiler", "SPPowerDataType"], capture_output=True, text=True, timeout=5
            )
    ...
    def get_aggregate_metrics(self, snapshots: List[HardwareSnapshot]) -> Dict:
        ...
        thermal_throttling = any(
            state in ["moderate", "heavy", "critical"] for state in thermal_states
        )
    ...
    def detect_speed_decay(
        self, baseline_speed: float, current_speed: float, threshold: float = 0.2
    ) -> bool:
