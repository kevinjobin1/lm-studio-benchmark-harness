...
class TypeScriptExecutor:
    ...
    def _check_eslint(self) -> bool:
        ...
        try:
            result = subprocess.run(["eslint", "--version"], capture_output=True, timeout=5)
            return result.returncode == 0
        except:
            return False
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                result = subprocess.run(
                    ["ts-node", str(ts_file)], capture_output=True, text=True, timeout=timeout
                )
    ...
    def check_types(self, code: str) -> Tuple[bool, List[str]]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                result = subprocess.run(
                    ["tsc", "--noEmit", str(ts_file)], capture_output=True, text=True, timeout=10
                )
    ...
    def lint_code(self, code: str) -> Tuple[bool, List[str]]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                result = subprocess.run(
                    ["eslint", str(ts_file)], capture_output=True, text=True, timeout=10
                )
                if result.returncode == 0:
                    return True, []
                else:
...
class CodeScorer:
    ...
    def _detect_failures(
        self, code: str, exec_output: str, type_errors: List[str]
    ) -> List[FailureType]:
        """Detect specific failure patterns."""
        failures = []
...
class ReasoningScorer:
    ...
    def score_reasoning(self, response: str, expected_concepts: List[str]) -> float:
        ...
        has_steps = any(
            ...
            for marker in ["step", "first", "then", "finally", "1.", "2.", "3."]
        )
        if has_steps:
            base_score *= 1.1
        ...
        has_tradeoffs = any(
            ...
            for word in ["trade-off", "pro", "con", "advantage", "disadvantage"]
        )
        if has_tradeoffs:
            base_score *= 1.05
...
class DeveloperRealismScorer:
    ...
    def compute_developer_score(
        components: ScoreComponents, latency_score: float, verbosity_penalty: float = 0.0
    ) -> float:
        """
...
class ComprehensiveEvaluator:
    ...
    def evaluate(
        ...
        if category == "code":
            ...
            if expected_concepts := prompt_data.get("expected_keywords"):
                components.reasoning_quality = self.reasoning_scorer.score_reasoning(
            ...
            if expected_concepts := prompt_data.get("expected_keywords"):
                ...
                components.correctness = (
                    components.reasoning_quality
                )  # For reasoning, correctness = reasoning quality
            ...
            components.instruction_compliance = (
                self.instruction_scorer.score_instruction_compliance(response, constraints)
            )
        ...
        stat_score = StatisticalScore(
            overall_score, 0.0, overall_score, overall_score, overall_score, 1
        )
    ...
    def evaluate_with_variance(
        self, responses: List[str], prompt_data: Dict, category: str
    ) -> EvaluationResult:
        ...
        mean_components = ScoreComponents(
            ...
            instruction_compliance=statistics.mean(
                [e.score_components.instruction_compliance for e in evaluations]
            ),
            reasoning_quality=statistics.mean(
                [e.score_components.reasoning_quality for e in evaluations]
            ),
            code_executability=statistics.mean(
                [e.score_components.code_executability for e in evaluations]
            ),
