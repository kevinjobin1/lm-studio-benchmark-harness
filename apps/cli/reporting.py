...
class ReportGenerator:
    ...
    def _generate_html(self, summary: Dict[str, Any], output_file: Path):
        ...
        html = f"""<!DOCTYPE html>
<html>
<head>        <title>Model Lens Benchmark Report</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>🔬 Model Lens Benchmark Report</h1>
        ...
        for framework in ["lm_eval", "openbench", "custom"]:
            if framework in comparison_results:
                if framework == "custom":
                    ...
                    if (
                        "lm_eval" in comparison_results[framework]
                        and "results" in comparison_results[framework]["lm_eval"]
                    ):
                        all_benchmarks.update(
                            comparison_results[framework]["lm_eval"]["results"].keys()
                        )
...
"""
        ...
        html += """
    <div class="footer">
        <p>Generated by Model Lens - Framework Comparison Mode</p>
    </div>
</body>
</html>
"""
    ...
    def _extract_score(
        self, comparison_results: Dict[str, Any], framework: str, benchmark: str
    ) -> Optional[float]:
            ...
            if framework == "custom":
                if "custom" in comparison_results and "benchmarks" in comparison_results["custom"]:
                    if benchmark in comparison_results["custom"]["benchmarks"]:
                        metrics = comparison_results["custom"]["benchmarks"][benchmark].get(
                            "metrics", {}
                        )
                        if metrics:
                ...
                if framework in comparison_results and "lm_eval" in comparison_results[framework]:
                    if "results" in comparison_results[framework]["lm_eval"]:
                        ...
                        mapping = (
                            ...
                            if framework == "lm_eval"
                            else OPENBENCH_TASK_MAPPING
                        )
