...
def _normalize_trace_ids(raw: dict[str, Any]) -> list[str]:
    """Normalize trace_ids/trace_id field to a list of strings.
    
    Handles:
    - trace_ids (list) → as-is
    - trace_id (string) → wrapped in list
    - neither → empty list
    """
    trace_val = raw.get("trace_ids", raw.get("trace_id"))
    if isinstance(trace_val, str):
        return [trace_val]
    if isinstance(trace_val, list) and all(isinstance(v, str) for v in trace_val):
        return trace_val
    return []
...
def normalize_result(raw: dict[str, Any], source_file: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        metrics = {
            "coding_score": _safe_float(
                _get_first(metrics_raw, "coding_score", "coding")
            ),
            "reasoning_score": _safe_float(
                _get_first(metrics_raw, "reasoning_score", "reasoning")
            ),
            "instruction_score": _safe_float(
                _get_first(
                    metrics_raw, "instruction_score", "instruction", "instructions"
                )
            ),
            "frontend_score": _safe_float(
                _get_first(metrics_raw, "frontend_score", "frontend")
            ),
            "math_score": _safe_float(_get_first(metrics_raw, "math_score", "math")),
            "debugging_score": _safe_float(
                _get_first(metrics_raw, "debugging_score", "debugging")
            ),
            "overall_score": _safe_float(
                _get_first(metrics_raw, "overall_score", "overall", "accuracy")
            ),
        }
        ...
        performance = {
            "tokens_per_sec": _safe_float(
                _get_first(perf_raw, "tokens_per_sec", "tokens_per_second")
            ),
            "normalized_tps": _safe_float(
                _get_first(
                    perf_raw,
                    "normalized_tps",
                    default=_safe_float(_get_first(perf_raw, "tokens_per_sec")) * 0.95,
                )
            ),
            "ttft_ms": _safe_float(
                _get_first(perf_raw, "ttft_ms", "ttft", "time_to_first_token")
            ),
            "total_latency_ms": _safe_float(
                _get_first(perf_raw, "total_latency_ms", "total_time", "latency")
            ),
            "memory_pressure_mb": _safe_float(
                _get_first(perf_raw, "memory_pressure_mb", "memory_mb", "ram_usage_mb")
            ),
        }
        ...
        stats = {
            "mean": _safe_float(
                _get_first(stats_raw, "mean", default=metrics["overall_score"])
            ),
            ...
            "median": _safe_float(
                _get_first(stats_raw, "median", default=metrics["overall_score"])
            ),
            ...
            "coefficient_of_variation": _safe_float(
                _get_first(stats_raw, "coefficient_of_variation", "cv", default=0.1)
            ),
        }
        ...
        return {
            ...
            "trace_ids": _normalize_trace_ids(raw),
        }
        ...
        runs.append(
            {
                ...
                "seed": 42 + i,
                "trace_ids": [f"demo_trace_{m['name'].replace('/', '_')}_{i}"],
            }
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate dashboard results.json from benchmark output"
    )
    parser.add_argument(
        "--results-dir", default="./results", help="Path to benchmark results directory"
    )
    parser.add_argument(
        "--output",
        default="./public/results.json",
        help="Output path for results.json (relative to CWD, typically the dashboard/ directory)",
    )
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Generate demo data instead of loading real results",
    )
    ...
    print(
        f"✓ Wrote {len(data['runs'])} runs ({data['total_models']} models) to {output_path}"
    )
