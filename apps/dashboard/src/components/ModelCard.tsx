...
function getTierLabel(overall: number): string {
  if (overall >= 0.9) return "S-TIER";
  if (overall >= 0.8) return "A-TIER";
  if (overall >= 0.65) return "B-TIER";
  if (overall >= 0.5) return "C-TIER";
  return "F-TIER";
}
...
export default function ModelCard({ result, rank }: ModelCardProps) {
  ...
  const speedClass =
    result.performance.tokens_per_sec > 65
      ? "metric-good"
      : result.performance.tokens_per_sec > 45
        ? "metric-ok"
        : "metric-slow";
  ...
  return (
    <article className={`model-card ${rank === 1 ? "model-card-top" : ""}`}>
        ...
        <div className="model-card-title">
          ...
          >
            ...
            <span className="chip chip-warning">
              {getTierLabel(result.metrics.overall_score)}
            </span>
          </div>
        </div>
      </div>
      ...
      <a
        href={`/model/${encodeURIComponent(result.model)}`}
        className="model-card-overall"
      >
      ...
      <div className="model-card-metrics">
        <div className="metric-row">
          ...
          <span className="metric-val">
            {(result.metrics.coding_score * 100).toFixed(0)}%
          </span>
        </div>
        <div className="metric-row">
          ...
          <span className="metric-val">
            {(result.metrics.reasoning_score * 100).toFixed(0)}%
          </span>
        </div>
        <div className="metric-row">
          ...
          <span className="metric-val">
            {(result.metrics.instruction_score * 100).toFixed(0)}%
          </span>
        </div>
      </div>
      ...
      <div className="model-card-perf">
        <div className={`perf-badge ${speedClass}`}>
          <span className="perf-value">
            {result.performance.tokens_per_sec.toFixed(0)}
          </span>
          <span className="perf-unit">tok/s</span>
        </div>
        <div className="perf-badge">
          <span className="perf-value">
            {result.performance.ttft_ms.toFixed(0)}
          </span>
      ...
      <p className="model-card-personality">
        {result.stats.runs} runs &middot; CI [
        {result.stats.confidence_95
          ? `${(result.stats.confidence_95[0] * 100).toFixed(1)}%, ${(result.stats.confidence_95[1] * 100).toFixed(1)}%`
          : "N/A"}
        ]
      </p>
    </article>
